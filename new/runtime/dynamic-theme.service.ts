...
export class ThemingService {
    ...
    primaryThemeColor = signal<string>(DEFAULT_PRIMARY);
    tertiaryThemeColor = signal<string>(DEFAULT_TERTIARY);
    ...
    constructor() {
        effect(() => {
            ...
            for (const [property, [lightArgb, darkArgb]] of Object.entries(properties)) {
                target.style.setProperty(property, `light-dark(${hexFromArgb(lightArgb)}, ${hexFromArgb(darkArgb)})`);
            }
        });
    }
}
