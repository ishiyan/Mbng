...
export const t3ExponentialMovingAverageMnemonic =
  ...
  if (guardLength(params)) {
    ...
    return 't3ema('.concat(Math.floor(p.length).toString(), ', ', p.vFactor.toFixed(3), p.firstIsAverage ? ', sma' : '',
      componentPairMnemonic(p.barComponent, p.quoteComponent), ')');
  } else {
    const p = params as T3ExponentialMovingAverageSmoothingFactorParams;
    return 't3ema('.concat(p.smoothingFactor.toFixed(3), ', ', p.vFactor.toFixed(3),
      componentPairMnemonic(p.barComponent, p.quoteComponent), ')');
  }
};

/** __T3 Exponential Moving Average__ line indicator computes the xxx, moving average (_T3EMA_).
 *
 */
