...
describe('ExponentialMovingAverage', () => {
  ...
  it('should return expected mnemonic', () => {
    ...
    expect(ema.getMnemonic()).toBe('ema(7, sma)');
    ema = new ExponentialMovingAverage({length: 7, firstIsAverage: false});
    expect(ema.getMnemonic()).toBe('ema(7)');
    ema = new ExponentialMovingAverage({smoothingFactor: 0.12345});
    expect(ema.getMnemonic()).toBe('ema(0.123)');
  });
