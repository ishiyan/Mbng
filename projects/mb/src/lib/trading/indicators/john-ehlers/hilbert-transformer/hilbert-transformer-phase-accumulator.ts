...
export class HilbertTransformerPhaseAccumulator implements HilbertTransformerCycleEstimator {
  ...
  public update(sample: number): void {
    ...
    if (this.isPrimed) {
      ...
      this.quadrature = ht(this.detrended) * amplitudeCorrectionFactor;
      this.inPhase = this.detrended[quadratureIndex];

      // Exponential moving average smoothing.
      const smoothedInPhase = this.emaQuadratureInPhase(this.inPhase, this.smoothedInPhasePrevious);
      const smoothedQuadrature = this.emaQuadratureInPhase(this.quadrature, this.smoothedQuadraturePrevious);
      ...
      this.period = instantaneousPeriod(this.deltaPhase, periodPrevious);
      ...
      this.quadrature = ht(this.detrended) * amplitudeCorrectionFactor; // count >= 16
      this.inPhase = this.detrended[quadratureIndex];
      if (this.smoothingLengthPlus2HtLengthMin2 === this.count) { // count == 16
        this.smoothedInPhasePrevious = this.inPhase;
        this.smoothedQuadraturePrevious = this.quadrature;
        return;
      }

      const smoothedInPhase = this.emaQuadratureInPhase(this.inPhase, this.smoothedInPhasePrevious); // count >= 17
      const smoothedQuadrature = this.emaQuadratureInPhase(this.quadrature, this.smoothedQuadraturePrevious);
      this.smoothedInPhasePrevious = smoothedInPhase;
      this.smoothedQuadraturePrevious = smoothedQuadrature;
      const phase = instantaneousPhase(smoothedInPhase, smoothedQuadrature, this.phasePrevious);
      ...
      if (this.smoothingLengthPlus2HtLengthMin1 < this.count) { // count > 17
        this.period = this.emaPeriod(this.period, periodPrevious);
        this.isPrimed = true;
      }
    }
