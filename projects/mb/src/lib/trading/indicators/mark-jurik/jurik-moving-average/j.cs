...
namespace Mbst.Indicators
{
    ...
    public sealed class JurikMovingAverage : Indicator, ILineIndicator
    {
        ...
        private double jmaValueBufferPrev;
        private double fC0BufferCur, fC0BufferPrev;
        private double fA8BufferCur, fA8BufferPrev;
        private double fC8BufferCur, fC8BufferPrev;
        private double[] list = new double[128];
        private double[] ring1 = new double[128];
        private double[] ring2 = new double[11];
        private double[] buffer = new double[62];
        ...
        public override void Reset()
        {
            lock (updateLock)
            {
                primed = false;
                value = double.NaN;
                jmaValueBufferPrev = 0;
                fC0BufferCur = 0; fC0BufferPrev = 0;
                fA8BufferCur = 0; fA8BufferPrev = 0;
                fC8BufferCur = 0; fC8BufferPrev = 0;
        ...
        public double Update(double sample)
        {
            ...
            lock (updateLock)
            {
                ...
                if (loopParam > N30)
                {
                    ...
                    for (int i = highLimit; i >= 0; i--)
                    {
                        ...
                        if (loopCriteria <= 30)
                        {
                            ...
                            if (loopCriteria == 30)
                            {
                                ...
                                fA8BufferCur = ((series - buffer[loopParam - upShift]) *
                                    (1 - dValue) / rightPart) +
                    ...
                    if (loopCriteria > 30)
                    {
                        jmaTempValue = jmaValueBufferPrev;
                        powerValue = pow(lengthDivider, dValue);
                        squareValue = pow(powerValue, 2);
                        fC0BufferCur = ((1 - powerValue) * series) +
                            (powerValue * fC0BufferPrev);
                        fC8BufferCur = ((series - fC0BufferCur) *
                            (1 - lengthDivider)) + (lengthDivider * fC8BufferPrev);
                        fA8BufferCur = (phaseParam * fC8BufferCur +
                            fC0BufferCur - jmaTempValue) *
                            (powerValue * (-2.0) + squareValue + 1) +
                            (squareValue * fA8BufferPrev);
                        jmaTempValue += fA8BufferCur;
                    }
                    jmaValue = jmaTempValue;
                    jmaValueBufferPrev = jmaValue;
                    fC0BufferPrev = fC0BufferCur;
                    fC8BufferPrev = fC8BufferCur;
                    fA8BufferPrev = fA8BufferCur;
                    return jmaValue;
                }
