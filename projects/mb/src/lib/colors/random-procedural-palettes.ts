 ...
 */
export const randomPalette = (numberOfSwatches: number): string[] => {
...
const randomDate = (now: number): any => new Date(now * Math.random()).getTime();

const rgbString = (color: any): string =>
  `rgb(${Math.floor(color.r * 255)}, ${Math.floor(color.g * 255)}, ${Math.floor(color.b * 255)})`;

const sampleFromColorScheme = (r1: number, r2: number, [color1, color2, color3]: any[]): any => ({
...
const generateColor = (time: number): any => ({
...
const generateColors = (n: number, [color1, color2, color3]: any[]) => {
