...
export class TreemapComponent implements OnChanges {
  ...
  readonly strokeFunc = input<HierarchyTreeStrokeFunction>(transparentStroke);
  ...
  readonly rectangleRatio = input<number>(defaultRectangleRatio);
  ...
  public render(): void {
    ...
    const squarify = (d: HierarchyTreeNode) => {
      ...
      return d3.treemap<HierarchyTreeNode>()
        .tile(d3.treemapSquarify.ratio(1))
        .size([w / this.rectangleRatio(), h])
        .padding(this.padding())
        .round(false)(rootNode);
    };
