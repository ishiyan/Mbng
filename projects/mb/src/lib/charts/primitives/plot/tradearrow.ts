...
export const tradearrow = function (accessor_trade: any, plot: any, plotMixin: any) { // Injected dependencies
  return function () { // Closure function
    var p = {} as any, // Container for private, direct access mixed in variables
      dispatch = d3.dispatch('mouseenter', 'mouseout'),
      y = function (d: any) { return p.yScale(p.accessor.p(d)); },
      svgArrow = arrow().orient(function (d: any) { return p.accessor.typ(d) === 'buy' ? 'up' : 'down'; }),
      arrowGenerator: any;
     ...
     *          a corresponding `svgArrow` orient value is returned.
     */
    ...
    function binder() {
      svgArrow.x(function (d: any) { return p.xScale(p.accessor.t(d)); }).y(y);
    ...
    function findNearest(d: any, x: any) {
      ...
      return d.map(function (d: any, i: any) { return { d: d, i: i, x: p.xScale(p.accessor.t(d)) }; })
    ...
    function typesToClasses(data: any) {
      return data.map(function (d: any) { return p.accessor.t(d); }).reduce(function (prev: any, cur: any) {
        if (prev[cur] === undefined) prev[cur] = function (d: any) { return cur === p.accessor.t(d); };
        return prev;
      }, {});
    }
