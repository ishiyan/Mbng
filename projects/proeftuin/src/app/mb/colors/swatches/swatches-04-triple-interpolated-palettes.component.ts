...
const tripletsMaterial: string[][] = [
  ['#2962FF', '#00BFA5', '#AEEA00'], // a7 blue, a7 teal, a7 lime
  ['#3D5AFE', '#00C853', '#FFD600'], // a4 indigo, a7 green, a7 yellow
  ['#AA00FF', '#00C853', '#FFD600'], // a7 purple, a7 green, a7 yellow
  ['#AA00FF', '#00E676', '#FFD600'], // a7 purple, a4 green, a7 yellow
  ['#AA00FF', '#00BFA5', '#AEEA00'], // a7 purple, a7 teal, a7 lime
  ['#AA00FF', '#1DE9B6', '#AEEA00'], // a7 purple, a4 teal, a7 lime
  ['#AA00FF', '#00C853', '#2962FF'] // a7 purple, a7 green, a7 blue
];
...
@Component({
  selector: 'app-mb-swatches-04-triple-interpolated-palettes',
  templateUrl: './swatches-04-triple-interpolated-palettes.component.html',
  styleUrls: ['./swatches-04-triple-interpolated-palettes.component.scss'],
...
export class Swatches04TripleInterpolatedPalettesComponent {
  ...
  palettes: string[][] = this.tripleInterpolatedPalettes(this.numberOfSwatches);
  selectedPalette: string[] = this.palettes[this.selectedIndex];

  palettesMaterial: string[][] = this.tripleInterpolatedPalettesMaterial(this.numberOfSwatchesMaterial);
  ...
  set paletteLength(value: number) {
    ...
    this.palettes = this.tripleInterpolatedPalettes(this.numberOfSwatches);
  ...
  set paletteLengthMaterial(value: number) {
    ...
    this.palettesMaterial = this.tripleInterpolatedPalettesMaterial(this.numberOfSwatchesMaterial);
    this.selectedPaletteMaterial = this.palettesMaterial[this.selectedIndexMaterial];
  }

  tripleInterpolatedPalettes(numberOfSwatches: number): string[][]{
  ...
  tripleInterpolatedPalettesMaterial(numberOfSwatches: number): string[][]{
    const palettes: string[][] = [];

    for (let i = 0; i < tripletsMaterialCount; ++i){
      const tr = tripletsMaterial[i];
      palettes.push(tripleInterpolatedPalette(tr[0], tr[1], tr[2], numberOfSwatches));
    }
  ...
  private linearSwatches(): string[][] {
    ...
    for (let i = 2; i < 20; ++i) {
      linearPalette.push(tripleInterpolatedPalette('#0000ff', '#00ff00', '#ffff00', i));
    }
